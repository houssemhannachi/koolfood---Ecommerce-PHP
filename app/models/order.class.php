...
class Order extends Controller
{
	...
	public function save_order($POST, $ROWS, $user_url, $sessionid)
	{
		...
		if (is_array($ROWS) && count($this->errors) == 0) {
			...
			//$city_obj = $states->get_city($POST['city']);
			$data['city'] = $POST['city'];
			...
			$query = "insert into orders (description,user_url,delivery_address,total,state,city,zip,tax,shipping,date,sessionid,mobile_phone,home_phone) 
			values 
